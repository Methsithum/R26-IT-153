...
export default function RiskPanel({
  ...
  if (!risk_level) {
    return (
      <section className="cp-panel">
        <h2 className="cp-panel-title">Academic Risk</h2>
        <p className="cp-placeholder">Run a prediction to see risk.</p>
      </section>
    );
  }

  const meta = RISK_META[risk_level] ?? { color: 'var(--text-dim)', text: '' };
  ...
  const bars = [
    { label: 'Low', value: prob_low ?? 0, color: 'var(--green)' },
    { label: 'Medium', value: prob_medium ?? 0, color: 'var(--orange)' },
    { label: 'High', value: prob_high ?? 0, color: 'var(--red)' },
  ];

  return (
    <section className="cp-panel rp">
      <h2 className="cp-panel-title">Academic Risk</h2>

...
export default function CareerGauge({ score }) {
  ...
  const stroke = hasScore ? colorForScore(score) : 'var(--border)';

  return (
    <section className="cp-panel cg">
      <h2 className="cp-panel-title">Career Readiness</h2>
      ...
      <div className="cg-gauge">
        <svg viewBox="0 0 180 180" className="cg-svg" role="img"
          ...
          {/* Track */}
          <circle cx={C} cy={C} r={R} fill="none" stroke="var(--bg-panel-alt)" strokeWidth="14" />
          ...
          <circle
            ...
            className="cg-arc"
          />
          <text x={C} y={C - 2} textAnchor="middle" className="cg-num" fill="var(--text)">
            {hasScore ? score.toFixed(1) : '—'}
          </text>
          <text x={C} y={C + 20} textAnchor="middle" className="cg-unit" fill="var(--text-faint)">
            out of 100
          </text>
        </svg>
      </div>

...
// prediction yet (e.g. completed/missed tasks not sent to /schedule) fall
// back to "Medium" without claiming a confidence that doesn't exist.
export default function MonthGrid() {
  ...
  const sessionsByDate = useMemo(() => {
    ...
    assignments.forEach((a) => {
      ...
      map[a.deadlineDate].push({
        ...
        status: a.status,
        priority: schedule?.tasks?.[a.taskId]?.priority_label || "Medium",
  ...
  return (
    <div className="card p-5">
      ...
      <div className="flex items-center gap-3 mb-4 text-[11px] font-medium text-slate-500 dark:text-slate-400">
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {cells.map((day, i) => {
          ...
          const dominant = daySessions.reduce(
            (acc, s) => (!acc || PRIORITY_RANK[s.priority] > PRIORITY_RANK[acc] ? s.priority : acc),
            null
          );
            ...
            >
              ...
              <div className="flex flex-col gap-0.5 mt-1 min-w-0">
                {visibleSessions.map((s, si) => (
                  <span
                    key={si}
                    title={`${s.title} · ${s.priority} priority`}
                    className={`flex items-center gap-1 rounded-md px-1 py-0.5 text-[9px] leading-tight font-semibold text-white truncate ${
                      s.status === "completed" ? "bg-low-500" : PRIORITY_COLORS[s.priority].solid
                    }`}
                  >
          ...
          >
            <div className="mt-4 pt-4 border-t border-black/5 dark:border-white/10">
              ...
              {selectedSessions.length === 0 ? (
                <p className="text-sm text-slate-400">No assignments due on this day.</p>
              ) : (
                <div className="space-y-2">
                  {selectedSessions.map((session) => (
                    <div
                      key={session.id}
                      className="flex items-center justify-between gap-3 rounded-xl border border-black/5 dark:border-white/5 p-2.5"
                    >

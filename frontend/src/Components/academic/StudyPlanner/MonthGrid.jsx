...
import { ChevronLeft, ChevronRight, Pencil, X, HelpCircle } from "lucide-react";
    ...
    <div className="card p-5">
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {cells.map((day, i) => {
          ...
          const dominantEntry = daySessions.reduce(
            (acc, s) => (!acc || PRIORITY_RANK[s.priority] > PRIORITY_RANK[acc.priority] ? s : acc),
            null
          );
              ...
              {selectedSessions.length === 0 ? (
                ...
                <div className="space-y-2">
                  {selectedSessions.map((session) => (
                    <div
                      key={session.id}
                      className="rounded-xl border border-black/5 dark:border-white/5 p-2.5"
                    >
                      ...
                      {!session.isPredicted && (
                        <p className="text-[11px] text-slate-400 mt-1.5 pl-[18px]">
                          Priority not yet predicted — add this task to your weekly plan (Study Planner → Week) to get
                          an ML-based priority.
                        </p>
                      )}
                    </div>
                  ))}

...
### 8f. Exam-Prep Front-Loading Bug (Fixed)

**Real bug found live, reported by a student**: a "lab" exam 20 days out, with generous free time (3h/day) and no other competing tasks, showed real prep sessions *today* but **completely empty weeks for everything else, including the week the exam itself falls in** — exactly backwards from what an escalating-urgency curve should produce (light now, heaviest right before the exam).

**Root cause.** `buildMultiWeekExamPrepTasks()` (Section 8's exam-prep subsection) submits one task per `(exam, week)` pair — `exam-<id>-w0` (light), `exam-<id>-w1` (moderate), `exam-<id>-w2` (heaviest) — each carrying the exam's own real `deadline_date`, since a chunk still can't be studied *after* the exam. But `generate_rolling_schedule()` had no concept of a chunk also having an *earliest* eligible week — only a latest one (the deadline). With three tasks all sharing the same distant deadline and week 0 having plenty of idle capacity (nothing else competing for it), the greedy allocator happily placed w1's 2.25h and w2's 3.5h into week 0 immediately, alongside w0's own 1.5h — collapsing the whole three-week escalating curve into "cram it all in right now." Traced via the live network request/response for `/multi-week-schedule`: the request correctly sent three tasks with correct escalating hours (1.5h / 2.25h / 3.5h); the response's `weeks_allocated` showed `[0]` for all three.

This had gone uncaught by `TestThreeWeekHorizonFullCorrectness` (Section 8d/8e's own regression sweep) because that scenario deliberately used *scarce* free time with several competing assignments — early weeks' capacity was already consumed by other work, so later exam chunks were forced to spill into later weeks "by accident," masking the missing constraint. The bug only becomes visible with idle capacity and nothing else competing — exactly this student's real, mostly-empty account.

**Fix.** `TaskInput` (`task_schemas.py`) gains an optional `not_before_date` (ISO date, default `None` — no behavior change for ordinary assignments, which never set it). `generate_rolling_schedule()`'s per-week task-pool filter now also skips a task whose `not_before_date` is later than the current week's anchor, alongside the existing deadline check. `buildMultiWeekExamPrepTasks()` sets each chunk's `not_before_date` to that chunk's own real week-start date (`today + weekIdx*7` days) — so `w2` simply isn't eligible for scheduling until its own week actually begins, no matter how much idle capacity an earlier week has.

**Verification (live, real account, real backend).** Reproduced end-to-end first (registered a fresh account, inserted a real `exam_type: "lab"` document 20 days out via the real journal `exams` collection, let the real `syncFromJournal` pick it up — not a synthetic/seeded repro) before fixing, confirming the bug live via the actual `/multi-week-schedule` network payload. After the fix and a backend restart: the exam's own week (Sep 14–20) correctly shows 3.5h of "Lab Preparation" spread across it, the moderate week shows 2.25h, and today's week shows only its own 1.5h — matching the intended escalating curve exactly.

**Tests.** `TestFrontLoadingBugFix` (`tests/test_rolling_schedule.py`, 3 tests): each of a 20-day-out exam's three weekly chunks lands in its *own* week (`weeks_allocated == [expected_week]`), not all in week 0; the fix doesn't drop or duplicate any hours (full amount placed exactly once per chunk, no `overload_warning`); and a dedicated sanity check confirms the same scenario *does* reproduce the original bug when `not_before_date` is stripped — proving the regression test actually exercises the real failure mode, not an unrelated pass. Backend suite: 75 passed (72 prior + 3 new).
...
| **8f** — Exam-prep front-loading fix | Each of a multi-week exam-prep chunk's `not_before_date` is respected — a later, heavier week's chunk is never scheduled into an earlier week just because that week has idle capacity; the fix places every chunk's full hours exactly once, in its own week, with no `overload_warning`; a dedicated sanity check confirms the same scenario reproduces the original front-loaded-into-week-0 bug when `not_before_date` is stripped, proving the test exercises the real failure mode. | `tests/test_rolling_schedule.py::TestFrontLoadingBugFix` |
